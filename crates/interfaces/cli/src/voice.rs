...
use pocket::TTSModel;
use pocket::weights::download_if_necessary;
...
pub fn resolve_voice(model: &TTSModel, voice_spec: Option<&str>) -> Result<pocket::ModelState> {
    match voice_spec {
...
fn resolve_voice_spec(model: &TTSModel, spec: &str) -> Result<pocket::ModelState> {
    let spec = spec.trim();
...
fn resolve_predefined_voice(model: &TTSModel, name: &str) -> Result<pocket::ModelState> {
...
fn resolve_hf_voice(model: &TTSModel, url: &str) -> Result<pocket::ModelState> {
...
fn resolve_file_voice(model: &TTSModel, path: &PathBuf) -> Result<pocket::ModelState> {
    let ext = path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("")
...
fn resolve_base64_voice(model: &TTSModel, spec: &str) -> Result<pocket::ModelState> {
    ...
    let (audio, sample_rate) = pocket::audio::read_wav_from_bytes(&bytes)
        .context("Failed to parse WAV from base64 data")?;

    // Resample if needed
    let audio = if sample_rate != model.sample_rate as u32 {
        pocket::audio::resample(&audio, sample_rate, model.sample_rate as u32)?
    } else {
        audio
    };
