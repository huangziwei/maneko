...
use pocket::TTSModel;
use pocket::audio::{read_wav, write_wav};
use pocket::voice_state::init_states;
...
fn test_generate_with_pauses_adds_silence() {
    ...
    let clean_text = pocket::pause::strip_pause_markers(text_with_pause);
    ...
    let frames_after_eos = pocket::tts_model::estimate_frames_after_eos("Hello");
...
fn test_pause_module_integration() {
    use pocket::parse_text_with_pauses;
...
fn test_quantize_module_integration() {
    ...
    use pocket::{QuantizeConfig, QuantizedTensor};
